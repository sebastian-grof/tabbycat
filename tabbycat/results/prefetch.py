...
def populate_results(ballotsubs, tournament=None):
    ...
    bye_ballotsubs = [ballotsub for ballotsub in ballotsubs if ballotsub.debate.is_bye]
    ballotsubs = [ballotsub for ballotsub in ballotsubs if not ballotsub.debate.is_bye]

    for ballotsub in bye_ballotsubs:
        ballotsub._result = DebateResult(ballotsub)
    ...
    crosses = list(tournament.crossexamination_set.order_by('seq')) if tournament.pref('teams_in_debate') == 2 else []
    crosses_enabled = (
        tournament.pref('teams_in_debate') == 2 and
        tournament.pref('cross_examinations_enabled')
    )
    if not crosses_enabled:
        crosses = []
    ...
    for ballotsub in ballotsubs:
        ...
        result = DebateResult(
            ...
            criteria=criteria,
            crosses=crosses,
            using_cross_examinations=crosses_enabled,
        )
