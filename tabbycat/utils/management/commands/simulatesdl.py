...
class Command(BaseCommand):
    help = (
        "Runs an end-to-end smoke simulation for an SDL Format tournament "
        "(3 vs 3 with replies and cross-examinations, pre-allocated sides, "
        "per-adjudicator private-URL ballots). By default all database writes "
        "are rolled back."
    )
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            type=int,
            default=16,
            help="Number of teams to create. Must be even for a 2-team format.",
        )
        parser.add_argument(
            "--keep",
            action="store_true",
            help="Keep the generated tournament instead of rolling back all writes.",
        )
    ...
    def handle(self, *args, **options):
        ...
        if options["teams"] < 2:
            raise CommandError("--teams must be at least 2.")
        if options["teams"] % 2:
            raise CommandError("--teams must be even for a 2 vs 2 format.")
        ...
        if options["keep"]:
            summary = self._run_suite(options)
            self._write_summary(summary, kept=True)
            return

        with transaction.atomic():
            summary = self._run_suite(options)
            self._write_summary(summary, kept=False)
            transaction.set_rollback(True)

    def _run_suite(self, options):
        suffix = uuid4().hex[:8]
        user = User.objects.create_user(f"sdl-smoke-{suffix}", "", "sdl-smoke")

        team_count = options["teams"]
        return self._simulate_tournament(
            slug=f"sdl-smoke-{suffix}",
            name="SDL Smoke",
            preset=SDLFormatPreferences,
            team_count=team_count,
            speakers_per_team=3,
            adjudicator_count=max(1, team_count // 2),
            venue_count=max(1, team_count // 2),
            round_count=options["rounds"],
            user=user,
            institution_code=f"S{suffix[:7]}",
        )
    ...
    def _write_summary(self, summary, *, kept):
        self.stdout.write(self.style.SUCCESS("SDL smoke simulation passed."))
        self.stdout.write(
