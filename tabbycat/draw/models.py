...
def get_effective_side_allocation_mode(round):
    ...
    active_team_ids = round.active_teams.values_list('id', flat=True)
    has_active_allocations = round.teamsideallocation_set.filter(team_id__in=active_team_ids).exists()
    return 'preallocated' if has_active_allocations else 'postallocated'
